...
float find_avg_error(vector<string> &cluster, const int des_len){
    int length = 0;
    for (string v: cluster){
        length += v.length();
    }
    float avg_len = length / cluster.size();
    float avg_num_deleted = des_len - avg_len;
    float error_rate = avg_num_deleted / 100;
    return error_rate;
}

string _recon_final(vector<string> &cluster, const int des_len, int &origSize, int threshold){
    float error_rate = find_avg_error(cluster, des_len);
    if (error_rate <= 0.1){
      error_rate = 0.0;
    }
    float normal_factor = 0.25;
    float new_threshold = des_len * 3 * error_rate * normal_factor;
    // int precentile = 20;
    int precentile = 100;
    int num_to_take = precentile * cluster.size() / 100;
    vector<string> selected_cluster;
    for(int i = 0; i <num_to_take; i++){
        selected_cluster.push_back(cluster[i]);
    }

    vector<vector<int>> kmer_cluster;
    vector<bool>kmer_created;
    for (int i = 0; i < cluster.size(); i++){
         kmer_cluster.push_back(vector<int>());
         kmer_created.push_back(false);
     }
    int curr_dist = 0;
    int emd= cluster.size();
    string guess="";
    ...
    int count=0;
    vector<string> v;
      for (int i = 0 ; i < selected_cluster.size() - 1; i ++) {
         v.push_back(cluster[i]);
          // if (! kmer_created[i]){
          //     kmer_created[i] = true;
          //     kmer_cluster[i] = get_kmer(cluster[i], 2);
          // }
         for (int j = i + 1; j < selected_cluster.size();  j++) {
             v.push_back(cluster[j]);
              // if (! kmer_created[j]){
              //     kmer_created[j] = true;
              //     kmer_cluster[j] = get_kmer(cluster[j], 2);
              // }
             count++;
             // curr_dist = vector_distance(kmer_cluster[i], kmer_cluster[j]);
             // if (curr_dist < new_threshold){
             //     kmer_skips ++;
             //     v.pop_back();
             //     continue;
             // }
     ...
     // THREE
    count = 0;
    for (int i = 0 ; i < selected_cluster.size() - 2; i ++) {
         v.push_back(cluster[i]);
         for (int j = i + 1; j < selected_cluster.size() - 1; j++) {
             v.push_back(cluster[j]);
             for (int k = j + 1; k < selected_cluster.size(); k++) {
                 v.push_back(cluster[k]);
                 count++;
                  // curr_dist = vector_distance(kmer_cluster[i], kmer_cluster[j], kmer_cluster[k]);
                  // if (curr_dist < new_threshold){
                  //     kmer_skips ++;
                  //     v.pop_back();
                  //     continue;
    ...
    count=0;
    for (int i = 0 ; i < selected_cluster.size() - 3; i ++) {
        v.push_back(cluster[i]);
        for (int j = i + 1; j < selected_cluster.size() - 2; j++) {
            v.push_back(cluster[j]);
            for (int k = j + 1; k < selected_cluster.size() - 1; k++) {
                v.push_back(cluster[k]);
                for (int l = k + 1; l < selected_cluster.size(); l++) {
                    v.push_back(cluster[l]);
                    count++;
                    int scs_len = SCSNLenFast(v, des_len);
                    if (scs_len == des_len) {
                        vector<string> SCS_set = SCSNFast(v, des_len);
...
int main(int argc, char* argv[])
{
    ...
    bool reset = false;
    // float prob_values [100] = {0.1,0.11, 0.12, 0.13, 0.14, 0.15};
    string file_name = "0.18SCS/";
    time_t start, end, total_time;

    // std::cout << "a";
    int org = 0;
    float prob_value = 0.18;
    try {
      #pragma omp num_threads(3)
      #pragma omp parallel for private(start, end, total_time)
        for (int prob_value_index = 0; prob_value_index < 9; prob_value_index++) {
            file_name += "plmoaplo_m_";
            file_name +=  to_string(prob_value_index);
            // Guy: kmer_threshold = length * (2k-1) * eror_prob * TBD
            // For now k = 2 and TBD is played with
            float normal_factor = 0.25;
//           0.15 is a way to catch both lower error probs and still be not shit
            // int kmer_threshold = ceil(DES_LEN * 3 * 0.15 * tbd);
            clear_global_vars();
            // ofstream MyFile(file_name + std::to_string(prob_values[prob_value_index]) + ".txt", ios::app);
            ofstream MyFile(file_name + std::to_string(prob_value) + ".txt", ios::app);
//          ofstream MyFile(file_name + std::to_string(prob_values[prob_value_index]) + "fours" + ".txt", ios::app);
            reset = true;
            total_time = 0;
            double succes_rate=0.0;
            // float prob_value = prob_values[prob_value_index];
            // float prob_value = prob_value;
            ...
            for (int j = 0; j < 1; j++) {
                MyFile << "Cluster size :  " << clus_size << " Deletion Prob : " << prob_value << " Number of tests "
                       << num_of_test << " str len : " << design_len << " lower bound : " << 2.2 * prob_value * prob_value << " q: "
                       << q << endl << endl;
                int origSize = 0;
                // #pragma omp num_threads(4)
                // #pragma omp parallel for
                for (int aa = 0; aa < num_of_test; aa++) {
                    original = genRandomSeq(design_len, q);
                    createNoisyCluster(cluster, original, clus_size, prob_value);
...
//                cout << "---" << endl;
                    ...
                    if (aa % 10 == 0) {
                        cout << aa << endl;
                        // cout << "*******Kmer k = 2  and TBD  = " << tbd << "********" << endl;
                        cout << "*******SCS only top 20 ********" << prob_value_index << endl;
                        double res = (double) (total_dis_tests) / (aa + 1);
                        res = res / original.length();
                        cout << "Error rate: " << res << endl;
                        cout << "Succes rate: " << succes_rate / (aa + 1) << endl;
                        ...
                        cout << endl << endl;
                        double res_BMA = (double) (total_dis_tests_BMA) / (aa + 1);
                        res_BMA = res_BMA / original.length();
                        cout << "******* BMA ALG ********" << endl;

                        cout << "Error rate BMA: " << res_BMA << endl;
                        cout << "Succes rate BMA: " << succes_rate_BMA / (aa + 1) << endl;
                ...
                MyFile << "Sucess rate = " << succes_rate / num_of_test << endl;
                MyFile << "Time = " << total_time << endl;
                MyFile << "Kmer skips: " << kmer_skips << endl;
                MyFile << "----" << endl;

                cout << endl;
                double res = (double) (total_dis_tests) / (num_of_test);
                res = res / original.length();
                // MyFile << "*******Kmer k = 2  and TBD  = " << tbd << "********" << endl;
                cout << "*******SCS only top 20 ********" << endl;
                MyFile << "Error rate: " << res << endl;
                MyFile << "Succes rate: " << succes_rate / num_of_test << endl;
                MyFile << "Lucky: " << lucky << endl;
                MyFile << "Two: " << two << endl;
                MyFile << "Three: " << three << endl;
                MyFile << "Four: " << four << endl;
                MyFile << "Nothing: " << nothing << endl;
                MyFile << "SR histo: " << endl;
                for (int i = 0; i < 10; i++) {
                    //if(i%10==0)
                    //    cout << endl;
                    MyFile << i << " : " << SR_histo[i] << "\t";
                ...
                MyFile << endl << endl;
                MyFile << "two histo: " << endl;
                int ncr = NCR(clus_size, 2) + 1;
                for (int i = 0; i < ncr && i < HISTO_LEN; i++) {
                    if (i % 10 == 0)
                        MyFile << endl;
                    MyFile << i << " - " << pairs_histo[i] << "\t";
                }
                MyFile << endl << endl;
                MyFile << "three histo: " << endl;
                ncr = NCR(clus_size, 3) + 1;

                for (int i = 0; i < ncr && i < HISTO_LEN; i++) {
                    if (i % 10 == 0)
                        MyFile << endl;
                    MyFile << i << " - " << three_histo[i] << "\t";
                }
                MyFile << endl << endl;
                MyFile << "four histo: " << endl;
                ncr = NCR(clus_size, 4) + 1;

                for (int i = 0; i < ncr && i < HISTO_LEN; i++) {
                    if (i % 10 == 0)
                        MyFile << endl;
                    MyFile << i << " - " << four_histo[i] << "\t";
                }
                MyFile << endl << endl;
                ...
                res_BMA = res_BMA / original.length();
                MyFile << "Error rate BMA: " << res_BMA << endl;
                MyFile << "Succes rate BMA: " << succes_rate_BMA / num_of_test << endl;
                MyFile << "SR histo BMA: " << endl;
                for (int i = 0; i < 10; i++) {
                    //if(i%10==0)
                    //    cout << endl;
                    MyFile << i << " : " << SR_histo_BMA[i] << "\t";
                }
                MyFile << endl << endl;
